...
def makeScheduleHTML(date):
    ...
    file.write("var sel_day = -1;\n")
    file.write("function select(year, month, day, row, col){\n")
    file.write('if (sel_day != -1 && (list_con[sel_day] != $("textarea").val()))\n')
    file.write('alert("내용이 변경되었습니다");\n')
    file.write("else{\n")
    ...
    file.write('$("textarea").val(list_con[day]);\n')
